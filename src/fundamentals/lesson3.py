...
a string quote or double quote and can be letters or number or characters
"""
...
"""
String format isa method that allows strings or other data types 
...
def other_string_format(name: str, age: int):
    msg2 = 'My dog spot is 4 years old'
...
"""The f-string uses expressions within thr string directly. It
also can use the mini-programming as the format function.
Essentially the interpreter runs the format function itself.
The f-string also uses placeholder curly braces
"""


def basic_fstring(name: str, snack, num_snacks: int):
    print(f'My name is {name} and my favorite snack is {snack}')
    print(f'I eat {num_snacks} snack per day. I should cut down')
    like = f'I like {snack} type snacks'
    print(like)


basic_fstring('Axe', 'Chocolate', 5)
...
def basic_escape_seq():
    value_n: str = 'My name is Jay. \tHow are\n you doing today?'
    print(value_n)
