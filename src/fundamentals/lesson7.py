...
def basic_while():
    """ Basic while loop that has an intialized value,
...
def other_while():
    ...
    index of the string to provide output of a single character.
    """
...
def basic_for():
    """ A basic for loop that interates through a sequence,
...
def basic_range():
    ...
    and interates through for loop from 0 to 9
...
def other_range():
    """ This range function take both a start and stop
    arguments. It can accept negative values. The start
    value has to be smaller than the stop for positive values. Otherwise you
    get an empty result.
    """
...
# other_range()
    ...
    for able in range(4, 11):
        total += 1
        print(able)
    print(f'{total} is the number of values in the sequence')
...
def continue_for():
    ...
    if statement and begins another interation.
    """
...
def example_else():
    ...
    perform a block once the loop ends. The else statement
     will not execute if a break statement is used in the loop
     """
